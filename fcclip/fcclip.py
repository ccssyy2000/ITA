...
class FCCLIP(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        # _, self.train_num_templates, self.train_class_names = self.prepare_class_names_from_metadata(test_metadata,train_metadata)#临时加的
        ...
        self.train_data = {}
        self.load_csv('./text_top_vit_L/coco_image_70_vit_L_other.csv')
    def load_csv(self, csv_file):
            with open(csv_file, 'r') as file:
                ...
                for row in reader:
                    jpg_filename = row[0]
                    data_list = row[1:]
                    self.train_data[jpg_filename] = data_list
    ...
    def text_list(self, train_metadata, test_metadata):
        def split_labels(x):
            res = []
            for x_ in x:
                x_ = x_.replace(', ', ',')
        ...
        def extract_first_element(original_list):
            new_list = []
        ...
        if self.training:
            ...
            class_names = split_labels(test_metadata.stuff_classes)       
            class_names = [l for label in class_names for l in label]      
    ...
    def forward(self, batched_inputs):
        ...
        if self.training!=True: 
            top_class_or_imagetensor = images
        ...
        if self.training:
            the_image = []  
            for i  in range(bs):
                jpg_filename = batched_inputs[i]['file_name'] 
                jpg_filename = os.path.basename(jpg_filename)  
                class_list = self.train_data.get(jpg_filename) 
                class_list = eval(class_list[0])
                the_image.append(class_list)                 
            top_class_or_imagetensor = the_image               
        else:
            top_class_or_imagetensor = top_class_or_imagetensor  
        Train_or_Test = True if self.training else False  
        train_or_test_class = self.text_list(self.train_metadata, self.test_metadata)
        features, add_tgt = self.backbone(images.tensor,top_class_or_imagetensor,train_or_test_class,Train_or_Test)
        ...
        outputs = self.sem_seg_head(features, add_tgt)
        ...
        if self.training:
            if "instances" in batched_inputs[0]:
                ...
                targets = None
            a = targets[0]["labels"].shape
            
            value = a[0]
            with open('class_num_total.txt', 'a') as file:
    ...
    def instance_inference(self, mask_cls, mask_pred):
        ...
        labels = torch.arange(num_classes, device=self.device).unsqueeze(0).repeat(self.num_queries + 20, 1).flatten(0, 1)
